...
public interface BillDetailRepository extends JpaRepository<BillDetail, String> {
    ...
    @Query(value = """
            SELECT ROW_NUMBER() OVER( ORDER BY bide.created_date ASC ) AS stt, bi.id AS id_bill, pr.id As id_product, bide.id, pr.code AS code_product, pr.name AS product_name, co.name AS name_color, si.name AS name_size, so.name AS name_sole, ma.name AS name_material, ca.name As name_category, bide.price, bide.quantity  FROM bill_detail bide
