...
function BillDetailAccount() {
  ...
  useEffect(() => {
    BillHistoryClientApi.getByIdBill(id.id).then((res) => {
        const data = res.data.data.filter((item)=>item.statusBill !== "CHO_VAN_CHUYEN")
      setListBillHistory(data);
      setStatusPresent(data.statusBill)
    });
  ...
  return (
    <React.Fragment>
        <div className="bill-detail-account-header">
            <div style={{cursor:"pointer",color:"gray"}} onClick={()=> nav("/purchase")}> <FontAwesomeIcon icon={faChevronLeft} /> TRỞ LẠI</div>
      ...
      <Timeline minEvents={statusPresent !== 5? 5 : 1}  style={{borderBottom:"1px solid rgb(224, 224, 224)" }}  placeholder >
          ...
          style={{ width: '50px', height: '50px', fontSize: '12px' }}
            ...
            title={
              item.statusBill === "CHO_XAC_NHAN"
                ...
                ? (paymentMethod.method === "TIEN_MAT" ? "Đã xác nhận thông tin thanh toán":`Đã thanh toán ${formatMoney(bill.totalMoney)}`)
        ...
        <h2>Thanh toán</h2>
           <ul>
            <li className="line-payment-bill-account">
                <div className="text-payment-bill-account" >Tổng tiền hàng:</div> <div className="value-payment-bill-account">{formatMoney(bill.totalMoney)}</div>
            </li>
            <li className="line-payment-bill-account">
                <div className="text-payment-bill-account" >Phí vận chuyển:</div> <div className="value-payment-bill-account">{formatMoney(bill.moneyShip)}</div>
            </li>
            <li className="line-payment-bill-account">
                <div className="text-payment-bill-account" >Voucher của cửa hàng:</div > <div className="value-payment-bill-account">- {formatMoney( bill.itemDiscount)}</div>
            </li>
            <li className="line-payment-bill-account">
                <div className="text-total-bill-account" >Thành tiền:</div > <div className="value-total-bill-account">{formatMoney(bill.totalMoney + bill.moneyShip -bill.itemDiscount)}</div>
            </li>
           </ul>
           
      </div>
      <div className="payment-method-bill-account"> <h3 style={{marginRight:20}}>Phương thức thanh toán:</h3> <div> {paymentMethod.statusBill === "TIEN_MAT" ?"Thanh toán khi nhận hàng":"Chuyển khoản VNpay"}</div> </div>
    </React.Fragment>
  );
}
