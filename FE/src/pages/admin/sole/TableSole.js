...
const TableSole = () => {
  const [listSolePage, setListSolePage] = useState([]);
  const [totalPage, setTotalPage] = useState(0);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [selectedSoleId, setSelectedSoleId] = useState(null);
  // tạo đối tượng để hứng data inputs
  const [formData, setFormData] = useState({ name: "", status: "" });
  const [load, setLoad] = useState(false);
  // gọi hàm getOneById ở service sang
  const getOne = (id) => {
    SoleService.getOneById(id)
      .then((response) => {
  ...
  const update = (id, Sole) => {
    Modal.confirm({
      title: "Xác nhận",
      content: "Bạn có chắc chắn muốn upadate sản phẩm này?",
      onOk() {
        SoleService.updateSole(id, Sole).catch((err) => {
          console.log(err);
        });
  ...
  const add = (Sole) => {
    Modal.confirm({
      ...
      onOk() {
        SoleService.addSole(Sole).catch((err) => {
          console.log(err);
        });
  ...
  const loadData = async (page) => {
    SoleService.fetchAllSole(page)
      .then((res) => {
        setListSolePage(res.data.data.data);
  ...
  const handleSubmit = (e) => {
    ...
    if (selectedSoleId === null) {
      add(formData);
    } else {
      update(selectedSoleId, formData);
      setSelectedSoleId(null);
    }
  ...
  const columns = [
    {
      title: <div className="title-Sole">STT</div>,
      dataIndex: "stt",
      key: "stt",
    },
    {
      title: <div className="title-Sole">Tên Thể Loại</div>,
      dataIndex: "name",
      key: "name",
    },
    {
      title: <div className="title-Sole">Ngày Tạo</div>,
    ...
    {
      title: <div className="title-Sole">Ngày Cập Nhập</div>,
    ...
    {
      title: <div className="title-Sole">Trạng Thái</div>,
      dataIndex: "status",
      key: "status",
      render: (text) => {
    ...
    {
      title: <div className="title-Sole">Thao Tác</div>,
      dataIndex: "id",
      key: "actions",
      render: (id) => (
